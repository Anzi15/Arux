...
import { checkFieldValueExistsInDB, createDocumentInFirestore, deleteDocumentFromFirestore, getAllFirestoreDocuments, getFirestoreDocument, updateFirestoreDocument } from "./firebase-modules";
...
headerMsgForm.addEventListener("submit",async (e)=>{
    ...
    const updateTask = await updateFirestoreDocument("storeManagement","headerNotificationMsg",{value: newNotfication})
        ...
        inputValidator: async (value) => {
          if (!value) {
            ...
            if(!checkTask){
                await Swal.fire({
                    ...
                    inputValidator: async (value) => {
                        ...
                        if (!value) {
                            ...
                            const createTask = await createDocumentInFirestore("coupons", dataObj);
                            showAlert("success","Success","new coupon created successfully","Nice!").then(Response => {
                ...
                showAlert("error","coupon already exist","Try some other code","Alright")
            }
